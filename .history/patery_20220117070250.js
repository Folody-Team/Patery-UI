...
// get all properties style
const getStyle = (el) => {
  const style = window.getComputedStyle(el);
  const styleObj = [];
  for (let i = 0; i < style.length; i++) {
    styleObj[style[i]] = style.getPropertyValue(style[i]);
  }
  return styleObj;
};
// get all names of properties style
...
console.log(getStyleNames(document.body));
const elementTagName = "Patery";
