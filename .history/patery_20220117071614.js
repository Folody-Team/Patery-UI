...
const elementTagName = (el) => {
  const styleName = getStyleNames(document.body);
  const Patery = "Patery";
  for (let i = 0; i < styleName.length; i++) {
    if (styleName[i] === Patery) {
      return el.tagName;
    }
  }
      
};
console.log(elementTagName(document.body));
const P = document.querySelector(elementTagName(document.body));
